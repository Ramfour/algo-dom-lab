...
function findMax(arr, compare) {
  ...
  function selectionSort (arr, compare){
    ...
    for (let i = 0, l = result.length; i < l; i++){
      let minInd = i;
      for (let j = i + 1; j < l ; j ++){
        if (compare(result[j], result[minInd])) minInd = j;
      }
      [result[minInd], result[i]] = [result[i],result[minInd]]
    }
    return result
  }
  ...
  function merge (leftArr, rightArr, compare){
    let result = [], leftIndex = 0, rightIndex = 0, resultIndex = 0;
    ...
    // Оставшиеся элементы из левого массива
    ...
    // Оставшиеся элементы из правого массива
  ...
  function mergeSort (arr, compare = compareDesc){
    if (arr.length <= 1) return arr // Базовый случай

    // защита от мутации + середина массива (c округлением в меньшую сторону)
    const result = [...arr], l = result.length, mid = Math.floor(l / 2);
    const leftArr = [];  // левый массив
    const rightArr = []; // правый массив

    for (let i = 0; i < mid; i++){ // заполнение левого подмассива
      leftArr[i] = result[i];
    }
    
    for (let i = mid; i < l; i++){// заполнение правого подмассива
      rightArr[i - mid] = result[i];
    }

    return merge (mergeSort(leftArr, compare), mergeSort(rightArr, compare), compare)
  }
...
function createIsSortedAscContainer(pre) { // функция создания контейнера для проверки сортировки по возрастанию
...
function createIsSortedDescContainer(pre) { // функция создания контейнера для проверки сортировки по убыванию
...
const algorithmHandlers = { // объект-диспетчер: ключ — название алгоритма, значение — функция-обработчик
  ...
  'Selection Sort': () => renderCards(createResultObject(testData, arr => selectionSort(arr, compareDesc)), createArrayCard),
  'Merge Sort ↑':   () => renderCards(createResultObject(testData, arr => mergeSort(arr, compareDesc)),     createArrayCard),
  'Merge Sort ↓':   () => renderCards(createResultObject(testData, arr => mergeSort(arr, compareAsc)),      createArrayCard),
